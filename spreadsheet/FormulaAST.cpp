...
public:
    ...
    virtual double Evaluate(const std::function<double(Position*)>& function) const = 0;
...
public:
    ...
    double Evaluate(const std::function<double(Position*)>& args) const override {
        ...
        switch (type_) {
        case Type::Add:{
            res = lhs_->Evaluate(args) + rhs_->Evaluate(args);
            break;}
        case Type::Multiply:{
            res = lhs_->Evaluate(args) * rhs_->Evaluate(args);
            break;}
        case Type::Subtract:{
            ...
            break;}
        case Type::Divide:{
            //не уверен что корректно сравнение double с нулем(int), но иначе не работает
